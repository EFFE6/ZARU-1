...
const Contratistas: React.FC = () => {
  ...
  const [itemToDelete, setItemToDelete] = useState<Contratista | null>(null);
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/contratistas/${itemToDelete.id}`);
  ...
  const filteredData = data.filter(d => {
    ...
    return d.razonSocial.toLowerCase().includes(q) || d.nit.toLowerCase().includes(q);
  });
  ...
  return (
    <div className="db-content-card">
      <div className="db-toolbar">
        <p className="db-tab-description">Administra los contratistas de servicios de salud.</p>
      ...
      <div className="db-table-wrapper">
        <table className="db-table">
          <thead>
            <tr>
              <th>RAZÓN SOCIAL <ArrowUpDown size={13} className="db-sort-icon" /></th>
              <th>NIT</th>
              <th>ESPECIALIDAD</th>
              <th>REGIONAL</th>
              <th>ESTADO</th>
          ...
          <tbody>
            {loading ? (
              ...
              paginatedData.map(c => (
                <tr key={c.id}>
                  <td className="db-col-main">{c.razonSocial}</td>
                  <td className="db-col-nit">{c.nit}</td>
                  <td>{c.especialidad}</td>
                  <td><span className="db-regional-tag">{c.regional}</span></td>
                  <td>
                    <span className={`db-status-badge ${c.estado === 'Activo' ? 'activo' : 'inactivo'}`}>
                      <span className={`db-status-dot ${c.estado === 'Activo' ? 'activo' : 'inactivo'}`}></span>
                      {c.estado}
                    </span>
                  </td>
                  <td>
